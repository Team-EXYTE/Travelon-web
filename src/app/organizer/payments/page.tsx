...
const PaymentsPage: React.FC = () => {
		...
		const ongoingPayments = sortedPayments.filter(p => p.paymentStatus === "Ongoing");
		...
		return (
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
					...
					<h1 className="text-3xl font-bold text-gray-800 flex items-center gap-2">
						...
						<div className="mb-6 flex items-center justify-between">
							<div className="flex items-center gap-4 bg-white rounded-lg shadow border border-gray-200 px-4 py-2 w-auto">
								<label htmlFor="status-filter" className="text-sm font-medium text-black">Filter by status:</label>
								<select
									id="status-filter"
									value={filterStatus}
				...
				{showModal && (
					<div className="fixed inset-0 z-50 flex items-center justify-center" style={{background: 'rgba(255,255,255,0.5)', backdropFilter: 'blur(8px)'}}>
						<div className="bg-white/80 rounded-2xl shadow-2xl p-8 min-w-[340px] w-full max-w-md border border-gray-200">
							<h2 className="text-xl font-bold mb-6 text-gray-900">Request Payment</h2>
							<label htmlFor="ongoing-select" className="block text-sm font-medium text-gray-700 mb-3">Select Ongoing Payment:</label>
							<select
								id="ongoing-select"
					...
					<table className="min-w-full">
						<thead>
							<tr className="bg-gray-100 text-gray-700 text-sm">
								...
								<th className="py-3 px-4 text-center font-semibold">Participants</th>
								<th className="py-3 px-4 text-center font-semibold">Ticket Price ($)</th>
								<th className="py-3 px-4 text-center font-semibold">Status</th>
								<th className="py-3 px-4 text-center font-semibold">Total Payment ($)</th>
							</tr>
						</thead>
						<tbody>
							{filteredPayments.map((payment, idx) => (
								<tr key={idx} className="border-b last:border-b-0 hover:bg-gray-50 transition-colors">
									...
									<td className="py-3 px-4 flex items-center gap-2">
										...
										<span className="font-medium text-gray-800">{payment.eventName}</span>
									</td>
									<td className="py-3 px-4 text-center text-gray-700">{formatDate(payment.createdAt)}</td>
									<td className="py-3 px-4 text-center text-gray-700 font-semibold">{payment.participantCount}</td>
									<td className="py-3 px-4 text-center text-gray-700">{payment.ticketPrice}</td>
									<td className="py-3 px-4 text-center">
										<span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full border text-xs font-medium ${statusStyles[payment.paymentStatus]}`}>
									...
									<td className="py-3 px-4 text-center font-bold text-black">{payment.ticketPrice * payment.participantCount}</td>
								</tr>
							))}
						</tbody>
					</table>
